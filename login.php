...
<div class="mx-auto flex min-h-screen items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
    <div class="w-full max-w-md">
        ...
        <?php if ($message = flash('error')): ?><div class="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700"><?= sanitize($message) ?></div><?php endif; ?>
        <?php if ($message = flash('success')): ?><div class="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700"><?= sanitize($message) ?></div><?php endif; ?>
        <form method="post" class="mt-8 space-y-5">
            ...
            <button class="inline-flex w-full items-center justify-center rounded-xl bg-slate-900 px-6 py-4 text-sm font-semibold text-white hover:bg-slate-800">Login</button>
        </form>
